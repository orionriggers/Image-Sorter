...
try:
    ...
    _db_path = os.path.join(SCRIPT_DIR, "deep_browser.py")
    if os.path.isfile(_db_path):
        _db_spec = _ilu2.spec_from_file_location("deep_browser", _db_path)
...
_video_cache  = {}                 # {filepath: PIL.Image} — cache frame video
...
def get_video_frame(filepath, size=(640, 480)):
    ...
    if filepath in _video_cache:
        return _video_cache[filepath].copy()

    acquired = _ffmpeg_lock.acquire(timeout=4)
    if not acquired:
    ...
    try:
        ...
        if rc == 0 and os.path.exists(tmp_path) and os.path.getsize(tmp_path) > 0:
            try:
                ...
                return None
            # Salva in cache, rimuovi entry più vecchia se piena
            if len(_video_cache) >= _VIDEO_CACHE_MAX:
                _video_cache.pop(next(iter(_video_cache)))
            _video_cache[filepath] = img.copy()
            return img
...
def save_history(entries):
    """Salva lo storico su file JSON (max HISTORY_MAX entry)."""
    try:
        with open(HISTORY_FILE, "w", encoding="utf-8") as f:
            json.dump(entries[-HISTORY_MAX:], f, indent=2, ensure_ascii=False)
    except Exception:
        pass
        ...
        if action in ("moved", "moved_browser", "moved_timeline"):
            ...
            for orig in files:
                if os.path.join(dest_dir, os.path.basename(orig)) == filepath:
                    return entry
        # Per crop: files[0] è il file modificato (path attuale)
        elif action == "cropped":
...
class DuplicateFinder:
    ...
    def _run_sha(self, get_folders, scan_btn, result_lbl, status_lbl,
        ...
        folders = get_folders()
        if not folders:
            self._hud_alert("Attenzione", "Aggiungi almeno una cartella.",
                            parent=self.win)
            return
        lb.delete(0, tk.END)
        row_paths.clear()
    ...
    def _bulk_trash_ab(self):
        ...
        if not to_del:
            return
        if not self._hud_yesno("Cestina duplicati B",
    ...
    def _trash_row(self, idx, fpath, lb, row_paths):
        ...
        if not self._hud_yesno("Cestina", f"Spostare nel cestino?\n{fname}",
                 yes_label="Cestina", no_label="Annulla",
                 parent=self.win):
            return
        if send_to_trash(fpath):
    ...
    def _bulk_trash_lb(self, lb, row_paths, result_lbl):
        ...
        if not self._hud_yesno(
                ...
                f"Verra' conservato il primo file di ogni gruppo.",
                parent=self.win):
            return
        trashed = 0
...
class FolderBrowser:
    ...
    def __init__(self, parent, sorter):
        ...
        _lang = self.sorter.config.get("language","it")
        _nav_lbl = "Naviga" if _lang == "it" else "Navigate"
        self.win.title(f"{_nav_lbl}  v{VERSION}")
    ...
    def _build(self):
        ...
        tk.Label(qf, text=" | ",
                 font=("TkFixedFont", 7), bg=BG_COLOR,
                 fg=MUTED_COLOR).pack(side="left")
        self._show_tree_var = tk.BooleanVar(value=True)
        ...
        # Barra azioni cartella (visibile solo con cartella selezionata)
        self._action_frame_fixed = tk.Frame(w, bg=BG_COLOR)
        self._action_frame_fixed.grid(row=5, column=0, columnspan=2,
                                       sticky="ew", padx=0, pady=0)
        self._action_frame_fixed.grid_remove()

        # Status bar: due label affiancate (path grigio + nome ciano)
        self._status_frame = tk.Frame(w, bg=BG_COLOR)
    ...
    def _toggle_tree_panel(self):
        """Mostra/nasconde il pannello albero nel PanedWindow."""
    ...
    def _load_dirs_grid(self, folder, dirs):
        ...
        for i, dname in enumerate(all_names):
            ...
            short = dname if len(dname) <= 22 else dname[:20] + ".."
            ...
            tk.Label(cell, text=short, font=("TkFixedFont", 8),
                     bg=BG_COLOR, fg=_dir_fg).pack()
    ...
    def _load_thumbnails(self, folder):
        ...
        # Registra in cronologia navigazione (max 3, no duplicati)
    ...
    def _show_thumb_preview(self, event, filepath):
        """Mostra anteprima grande al passaggio del mouse sulla thumbnail."""
        self._hide_thumb_preview()
        if not os.path.isfile(filepath): return
        tip = tk.Toplevel(self.win)
        tip.withdraw()
        tip.overrideredirect(True)
        tip.configure(bg="#0a0f1a")
        tip.attributes("-topmost", True)
        self._thumb_tip = tip

        def _load(path=filepath, w=tip):
            try:
                from PIL import Image, ImageTk
                img = Image.open(path)
                img.thumbnail((280, 280), Image.Resampling.LANCZOS)
                tk_img = ImageTk.PhotoImage(img)
                def _show(i=tk_img, sz=img.size):
                    if not w.winfo_exists(): return
                    lbl = tk.Label(w, image=i, bg="#0a0f1a",
                                   relief="flat", bd=2,
                                   highlightbackground="#00c8ff",
                    ...
                    info = f"{name[:30]}\n{sz[0]}x{sz[1]}  {size_kb} KB"
                    tk.Label(w, text=info,
                             font=("TkFixedFont",7), bg="#0a0f1a",
                             fg="#4a6080", justify="center"
                             ).pack(pady=(0,4))
                    # Posiziona vicino al mouse ma dentro lo schermo
                    ...
                    mx = event.x_root + 16
                    my = event.y_root + 16
                    if mx + ww > sw: mx = event.x_root - ww - 8
                    if my + wh > sh: my = event.y_root - wh - 8
                    w.geometry(f"+{mx}+{my}")
                    w.deiconify()
            ...
            if fp not in self._selected_files:
                ...
                if cell:
                    ...
                    try:
                        ...
                        for w in cell.winfo_children(): w.config(bg="#1a4a1a")
                    except Exception: pass
        self._update_sel_bar()
    ...
    def _on_shift_arrow(self, e):
        ...
        for fp in old_set - new_set:
            ...
            if cell:
                try:
                    cell.config(bg=PANEL_COLOR)
                    for w in cell.winfo_children(): w.config(bg=PANEL_COLOR)
                except Exception: pass
        ...
        self._update_sel_bar()
        # Scrolla per rendere visibile la cella corrente
        def _do_scroll(cell=self._cell_refs.get(new_fp)):
            if not cell: return
            try:
                canvas   = self._thumb_canvas
                inner_h  = max(self._thumb_inner.winfo_height(), 1)
                canvas_h = max(canvas.winfo_height(), 1)
                if inner_h <= canvas_h: return  # tutto visibile
                # winfo_y() = posizione dentro _thumb_inner (parent diretto)
                cell_y = cell.winfo_y()
                cell_h = max(cell.winfo_height(), 10)
                view_top = canvas.yview()[0] * inner_h
                view_bot = view_top + canvas_h
                if cell_y < view_top + 8:
                    canvas.yview_moveto(max(0.0, (cell_y - 8) / inner_h))
    ...
    def _sel_convert_current(self):
        conv = [f for f in self._selected_files
                ...
                {".webp",".png",".bmp",".tiff",".gif",".avif"}]
        if conv: self.sorter._sel_convert_dialog(conv)

    def _update_sel_bar(self):
        """Aggiorna solo il contatore — non ricostruisce i widget."""
        ...
        if hasattr(self,"_sel_count_lbl") and self._sel_count_lbl.winfo_exists():
            self._sel_count_lbl.config(text=f"  {n} selezionati")
    ...
    def _clipboard_set(self, files, mode):
        ...
        self._status_msg(f"{verb} {len(files)} file — incolla con tasto destro", HUD_CYAN if mode == "copy" else WARNING)
    ...
    def _sel_move(self, dest_dir):
        ...
        for fpath in list(self._selected_files):
            ...
            try:
                ...
                # Registra nella history per Ctrl+Z
                self.sorter.history.append(
                    ("moved_browser", fpath, dest_path))
                moved += 1
    ...
    def _thumb_context_menu(self, event, filepath):
        ...
        _exif_targets = [f for f in targets
                         if os.path.splitext(f)[1].lower()
                         in {".jpg",".jpeg",".tiff",".tif",".webp"}]
        if _exif_targets:
            menu.add_separator()
            lbl_info = ("Info EXIF  (primo file)"
                        if len(_exif_targets)>1 else "Info EXIF")
            menu.add_command(label=lbl_info,
                             command=lambda: self.sorter._show_exif_info_popup(_exif_targets[0], parent=self.win))
            if _EXIF_EDITOR_AVAILABLE:
                lbl_exif = (f"Modifica EXIF ({len(_exif_targets)} file)..."
                            if len(_exif_targets)>1 else "Modifica EXIF...")
                menu.add_command(label=lbl_exif,
                                 command=lambda: _open_exif_editor(
                                     self.win, _exif_targets))
        # Conversione formato
        conv_targets = [f for f in targets
                        if os.path.splitext(f)[1].lower()
                        in {".webp",".png",".bmp",".tiff",".gif",".jpg",".jpeg",".avif"}]
        if conv_targets:
            menu.add_separator()
            menu.add_command(
        ...
        if not multi:
            menu.add_separator()
            menu.add_command(label="Proprieta...",
    ...
    def _show_tree_actions(self, path):
        """Mostra bottoni azione per una cartella."""
        self._clear_action_btns()
        # Aggiorna status bar con path + nome
        name = os.path.basename(path) or path
        ...
        af = self._action_frame_fixed
        af.config(bg="#152515")
        af.grid()
        af.columnconfigure(0, weight=1)

        # Riga unica: path/nome | bottoni azione | sep | rinomina/copia/cestina | x
        row1 = tk.Frame(af, bg="#152515")
        row1.grid(row=0, column=0, sticky="ew", pady=(2,2))

        # Bottoni principali
        tk.Button(row1, text="Sorgente",
        ...
        tk.Button(row1, text="File manager",
                  font=("TkFixedFont", 8), bg=ACCENT_COLOR, fg=HUD_CYAN,
                  relief="flat", padx=6, activebackground=HIGHLIGHT,
                  command=lambda p=path: open_in_filemanager(p)
                  ).pack(side="left", padx=(0,4), ipady=2)
        tk.Button(row1, text="Timeline",
                  font=("TkFixedFont", 8), bg="#2a1a4a", fg=HUD_CYAN,
                  relief="flat", padx=6, activebackground=HIGHLIGHT,
        ...
        # Separatore visivo
        tk.Frame(row1, bg=MUTED_COLOR, width=1).pack(
    ...
    def _rename_file_popup(self, filepath):
        ...
        win.deiconify()
        win.lift()
        win.after(50, lambda: (entry.focus_set(), entry.selection_range(0, tk.END)))
        # win.grab_set()  # rimosso: bloccava altri programmi
...
class KeypadPopup:
    ...
    def _assign_col_preset(self, col, name):
        ...
        if col == 0:
            ...
            for k in KEYS:
                try: os.makedirs(sorter._dest_path(k), exist_ok=True)
                except Exception: pass
        ...
        for ci in range(ncols):
            ...
            for row_keys in ROWS_LAYOUT:
                ...
                for gi, k in enumerate(row_keys):
                    ...
                    if _TKDND_OK:
                        _dnd_register(btn,
                            lambda dropped, key=k, p=pname:
                                self.sorter._dnd_set_dest(key, p, dropped))
                    if is_active:
    ...
    def _do_trash(self, filepath):
        ...
        try:
            ...
            self._hud_alert("Errore", f"Impossibile spostare nel cestino:\n{ex}",
                 parent=self.root)
            return
...
class CropOverlay:
    ...
    def __init__(self, sorter):
        ...
        ratio_labels = [r[0] for r in self.ASPECT_RATIOS]
    ...
    def _on_ratio_change(self, label):
        ...
        for name, aw, ah in self.ASPECT_RATIOS:
            if name == label:
                self._aspect_w = aw
                self._aspect_h = ah
                break
    ...
    def _init_selection(self):
        ...
        if remember and crop_pos and len(crop_pos) == 4:
            ...
            self._aspect_w = aw
            self._aspect_h = ah
            label = next((n for n, w, h in self.ASPECT_RATIOS
    ...
    def _save_crop(self, cropped, dest, advance, open_next=False):
        ...
        try:
            # Backup originale per undo nella cartella centralizzata
...
class SettingsDialog:
    ...
    def __init__(self, parent, sorter, initial_tab="preset"):
        ...
        self.win = tk.Toplevel(parent)
        self.win.withdraw()   # nasconde finché non è posizionata
        _cfg_lang = self.config.get("language","it")
        self.win.title(f"{T('settings', _cfg_lang)}  v{VERSION}")
        self.win.configure(bg=BG_COLOR)
        ...
        if _lang != "it":
            self.win.after(50, lambda: _translate_widgets(self.win, _lang))
        # Posiziona nel terzo sinistro dello schermo senza flash
        self.win.update_idletasks()
        sw = self.win.winfo_screenwidth()
        sh = self.win.winfo_screenheight()
        ww = min(780, sw * 2 // 5)
        wh = min(640, sh - 80)
        wx = sw // 5 - ww // 2   # circa 1/5 da sinistra
        wx = max(20, wx)
        wy = (sh - wh) // 2
    ...
    def _on_close_request(self, e=None):
        ...
        if self._dest_has_unsaved_changes():
            answer = self._hud_yesno(
                ...
                yes_label="Salva", no_label="Ignora",
                parent=self.win)
    ...
    def _build(self, initial_tab):
        ...
        for key, label in [
            ...
            ("history",  T("tab_history",  cfg_lang)),
            ("keys",     T("tab_keys",     cfg_lang)),
            ("deck",     T("tab_deck",     cfg_lang)),
            ("info",     T("tab_info",     cfg_lang)),
        ]:
        ...
        tk.Button(bot, text=T("close", cfg_lang),
                  font=("TkFixedFont", 9),
                  bg=ACCENT_COLOR, fg=TEXT_COLOR,
                  relief="flat", padx=16,
    ...
    def _build_preset_tab(self):
        ...
        tk.Label(f, text="* = preset attivo  |  colori: sidebar e Deck",
                 font=("TkFixedFont", 7), bg=BG_COLOR, fg=MUTED_COLOR,
        ...
        tk.Label(kf, text="Deck", font=("TkFixedFont", 9, "bold"),
        ...
        tk.Label(cols_row, text="Colonne (3+):", font=("TkFixedFont", 8),
                 bg=PANEL_COLOR, fg=MUTED_COLOR).pack(side="left", padx=(0,6))
        self._col_btns = {}
        for n in range(3, 9):   # 1 e 2 col gestite dal ciclo tasto Deck
    ...
    def _confirm_preset(self):
        ...
        if errors:
            self.sorter._show_toast(
                f"Permesso negato su {len(errors)} cartelle",
                color="#e74c3c", duration=3000)
        self.sorter._update_preset_label()
        self.sorter._build_sidebar()
        if self.sorter.keypad_popup:
            cols = get_keypad_cols(self.config)
            self.sorter._reopen_keypad(cols)
        self.win.destroy()
    ...
    def _move_up(self):
        ...
        self.sorter._build_sidebar()
        # Il Deck aggiornerà l'ordine alla prossima apertura
    ...
    def _new_preset(self):
        ...
        if not name or name in self.config["presets"]:
            self._hud_alert("Nome non valido", f"'{name}' non valido o gia' esistente.", parent=self.win)
            return
    ...
    def _copy_preset(self):
        ...
        name = sanitize_name(name)
        if not name or name in self.config["presets"]:
            self._hud_alert("Nome non valido", f"'{name}' non valido.", parent=self.win)
            return
    ...
    def _rename_preset(self):
        ...
        if not new or new in self.config["presets"]:
            self._hud_alert("Nome non valido", f"'{new}' non valido.", parent=self.win)
            return
    ...
    def _delete_preset(self):
        ...
        if len(self.config["presets"]) <= 1:
            self._hud_alert("Impossibile", "Deve esistere almeno un preset.", parent=self.win)
            return
        if not self._hud_yesno("Elimina preset",
                f"Eliminare il preset '{name}'?\n"
                "(Le cartelle su disco NON vengono toccate.)",
    ...
    def _set_keypad_cols(self, n):
        ...
        self.config["keypad_cols_configured"] = n   # valore utente, non sovrascritto dal ciclo
        save_config(self.config)
        self._refresh_col_btns()
        if self.sorter.keypad_popup:
            self.sorter._reopen_keypad(n)
    ...
    def _build_dest_tab(self):
        ...
        self._dup_lbl = tk.Label(hdr, text="", font=("TkFixedFont", 7),
                                  bg=PANEL_COLOR, fg=PRIVACY_RED, anchor="w")
        self._dup_lbl.grid(row=2, column=0, columnspan=3,
        ...
        self.win.after(150, self._check_dest_duplicates)
    ...
    def _check_dest_duplicates(self):
        """Controlla percorsi duplicati tra i tasti del preset corrente.
        Colora in rosso i campi percorso duplicati e aggiorna un indicatore."""
        # Raccoglie tutti i path non vuoti
        path_to_keys = {}
        for k, pv in self.path_vars.items():
            p = pv.get().strip()
            if p:
                path_to_keys.setdefault(p, []).append(k)

        duplicates = {p: keys for p, keys in path_to_keys.items() if len(keys) > 1}
        dup_keys = {k for keys in duplicates.values() for k in keys}

        # Aggiorna colore campo percorso
        inner = self._dest_inner
        for ri, k in enumerate(KEYS, start=1):
            slaves = inner.grid_slaves(row=ri, column=2)
            if slaves:
                entry = slaves[0]
                try:
                    if k in dup_keys:
                        entry.config(bg="#4a1a1a", fg="#ff8888")
                    else:
                        entry.config(bg=BG_COLOR, fg=MUTED_COLOR)
                except Exception:
                    pass

        # Aggiorna indicatore duplicati nell'header
        if hasattr(self, '_dup_lbl') and self._dup_lbl.winfo_exists():
            if duplicates:
                paths_str = "; ".join(
                    f"{'+'.join(keys)}: {os.path.basename(p) or p}"
                    for p, keys in duplicates.items())
                self._dup_lbl.config(
                    text=tk_safe(f"  Percorsi duplicati: {paths_str}"),
                    fg=PRIVACY_RED)
            else:
                self._dup_lbl.config(text="", fg=MUTED_COLOR)
    ...
    def _build_key_row(self, inner, ri, k):
        ...
        # Check duplicati al cambio percorso
        pv.trace_add("write", lambda *_: self.win.after(100,
                     self._check_dest_duplicates))

        # Drag & drop su badge e campo etichetta
        def _dest_dnd(dropped, key=k, pvar=pv, lvar=lv,
                       entry=path_entry, mvar=mv):
            if isinstance(dropped, list): dropped=dropped[0] if dropped else ""
            if not dropped: return
            if os.path.isfile(dropped): dropped=os.path.dirname(dropped)
            if not os.path.isdir(dropped): return
            folder_name = os.path.basename(dropped) or dropped
            pvar.set(dropped)
            mvar.set("custom")
            entry.config(state="normal")
            # Aggiorna etichetta solo se vuota
            if not lvar.get().strip():
                lvar.set(folder_name[:16])
            # Aggiorna self.slots e salva subito
            self.slots[key] = {
                "path":  dropped,
                "label": lvar.get().strip() or folder_name[:16],
            }
            self._apply_dest()
        if _TKDND_OK:
            for _w in [inner.grid_slaves(row=ri, column=0)[0],
                        inner.grid_slaves(row=ri, column=1)[0]]:
                _dnd_register(_w, _dest_dnd)
            _dnd_register(path_entry, _dest_dnd)
        ...
        def browse(key=k, pvar=pv, entry=path_entry, lvar=lv):
            try:
                ...
                folder = browse_folder_hud(self.win, title="Scegli cartella destinazione", initial_dir=initial, config=self.config)
                if folder:
                    pvar.set(folder)
                    ...
                    if lvar is not None:
                        ...
                        old_basename = os.path.basename(pvar.get().strip()) if pvar.get().strip() else ""
                        if not current_label or current_label == old_basename:
        ...
        def _browse_with_mkdir_impl(key, pvar, entry, initial_dir, lvar=None):
            ...
            def mkdir_action():
                ...
                try:
                    ...
                    self._hud_alert("Errore", f"Impossibile creare:\n{ex}", parent=dlg)
            ...
            def confirm():
                ...
                # o se coincide ancora col basename del percorso precedente
                if lvar is not None:
                    current_label = lvar.get().strip()
                    old_basename = os.path.basename(pvar.get().strip()) if pvar.get().strip() else ""
                    if not current_label or current_label == old_basename:
            ...
            # Posiziona nel terzo destro dello schermo
            dlg.update_idletasks()
            screen_w = self.win.winfo_screenwidth()
            screen_h = self.win.winfo_screenheight()
            pw = min(700, screen_w * 2 // 5)
            ph = min(560, screen_h - 80)
            px = screen_w * 3 // 5 - pw // 2   # circa 3/5 da sinistra
            px = min(px, screen_w - pw - 20)
            py = (screen_h - ph) // 2
    ...
    def _edit_idle_key(self, page_idx, key_idx):
        ...
        def _browse_img():
            f = browse_file_hud(win, title="Scegli immagine tasto")
            if f: img_var.set(f)
    ...
    def _apply_lang(self):
        """Salva la lingua e applica immediatamente la traduzione."""
        lang = self._lang_var.get()
        self.config["language"] = lang
        save_config(self.config)
        # Applica subito alle finestre aperte
        for w in [self.win, self.sorter.root]:
            try: _translate_widgets(w, lang)
            except Exception: pass
    ...
    def _build_private_tab(self):
        ...
        def _unlock_session():
            ...
            _refresh_list()
            status_lbl.config(
                text=f"Tutte le cartelle sbloccate per questa sessione.")
    ...
    def _build_history_tab(self):
        ...
        canvas.after(80, _populate_after)
        ...
        tk.Label(path_row,
                 ...
                 ).grid(row=1, column=0, columnspan=4, sticky="w", pady=(1,0))
    ...
    def _history_populate(self, frame, lang):
        ...
        for idx, entry in enumerate(reversed(entries)):
            ...
            row.columnconfigure(3, weight=1)  # colonna dettaglio espandibile
            ...
            # Col 1: Timestamp (fisso)
            tk.Label(row, text=entry.get("timestamp","")[:16],
                     font=("TkFixedFont", 7), bg=bg,
                     fg=MUTED_COLOR, width=14, anchor="w"
                     ).grid(row=0, column=1, padx=(2,4), pady=2)

            # Col 2: Tipo operazione (fisso, larghezza minima)
            ...
            # Col 3: Dettaglio file (espandibile)
            files = entry.get("files", [])
            dest  = entry.get("dest",  "")
            note  = entry.get("note",  "")
            fnames = ", ".join(os.path.basename(f) for f in files[:3])
            if len(files) > 3:
                fnames += f" +{len(files)-3}"
            dest_short = os.path.basename(dest) if dest else ""
            detail = tk_safe(fnames + (f" → {dest_short}" if dest_short else "") +
                             (f" ({note})" if note else ""))
            ...
            # Col 4: Bottone Annulla (fisso a destra)
            if can_undo:
                tk.Button(row, text=T("hist_undo", lang),
                          ...
                          ).grid(row=0, column=4, padx=(4,6), pady=2, ipady=1)
            else:
    ...
    def _history_do_undo(self, entry):
        ...
        if action in ("moved", "moved_browser", "moved_timeline"):
            ...
            dest_dir = dest
            for orig in files:
                d = os.path.join(dest_dir, os.path.basename(orig))
                if os.path.isfile(d):
                    try:
    ...
    def _history_clear(self, frame, lang):
        """Svuota lo storico dopo conferma."""
    ...
    def _build_info_content(self, inner, lang):
        ...
        # Bottone Privacy — apre il tab privato nascosto
        tk.Button(fr_rm, text="Privacy",
                  font=("TkFixedFont", 8), bg=ACCENT_COLOR, fg=TEXT_COLOR,
                  relief="flat", padx=10,
                  activebackground=HIGHLIGHT, activeforeground="white",
...
class ImageSorter:
    ...
    def _build_ui(self):
        ...
        _btn_list = [
            # pack(side="right") => ordine lista = dx->sx, visivo = sx->dx
            # Visivo: [Play] | Apri, Timeline, Impostazioni | Info EXIF | Originale, Estendi, -, +, Full Screen | Sidebar, S-Deck, Deck | Esci
            ...
            ("Timeline",    self._open_timeline,           "#2a1a4a",   "T"),
            ("Naviga",      self._open_new_source,         SUCCESS,      "O"),
            (None, None, None, None),
        ...
        for txt, cmd, col, key_hint in _btn_list:
            if txt is None:
                tk.Frame(hdr, bg=ACCENT_COLOR, width=1).pack(side="right", fill="y", padx=4, pady=6)
                continue
            if txt == "Play":
    ...
    def _move_to_preset_file(self, key, preset_name, filepath):
    ...
    def _finish_show_image(self, filepath, img):
        ...
        if _has_gps:
            ...
            _x = cw - _r - 8
            _y = _r + 8
            self.canvas.create_oval(_x-_r, _y-_r, _x+_r, _y+_r,
                                    fill="#00e060", outline="#003a18",
                                    width=1, tags="gps_dot")
            self.canvas.tag_bind("gps_dot", "<Button-1>",
                                 lambda e, fp=filepath: self._open_map_single(fp))
            self.canvas.tag_bind("gps_dot", "<Enter>",
                                 lambda e: self.canvas.config(cursor="hand2"))
            self.canvas.tag_bind("gps_dot", "<Leave>",
    ...
    def _show_image_menu(self, mx, my, filepath):
        ...
        if _conv_ext in {".webp",".png",".bmp",".tiff",".gif"}:
            btn("Converti in JPG", "#5a3a7a", lambda: self._convert_to_jpg(filepath))
        if _conv_ext in {".webp",".jpg",".jpeg",".png",".bmp",".tiff"}:
            btn("Converti in GIF", "#5a3a7a", lambda: self._convert_to_gif(filepath))
        ...
        if not is_video(filepath) and not is_pdf(filepath):
            btn("Info EXIF  [I]", "#1a2a3a",
                lambda: (win.destroy(), self._toggle_info()))
        if _EXIF_EDITOR_AVAILABLE and not is_video(filepath) and not is_pdf(filepath):
            btn("Modifica EXIF...", "#2a1a4a",
                lambda: _open_exif_editor(self.root, [filepath]))
    ...
    def _sel_convert_dialog(self, files):
        ...
        convertible = [f for f in files
            if os.path.splitext(f)[1].lower() in
            {".webp",".png",".bmp",".tiff",".gif",".jpg",".jpeg",".avif"}]
        if not convertible:
            return
        ...
        dlg.resizable(False, False)
        hud_apply(dlg)
        tk.Label(dlg, text=f"{len(convertible)} file selezionati — converti in:",
    ...
    def _open_map_single(self, filepath):
        ...
        if not _DEEP_BROWSER_AVAILABLE:
            ...
            return
        try:
            from deep_browser import get_exif_data, build_map
            import datetime
    ...
    def _open_timeline(self):
        """Apre la visualizzazione profonda/timeline — una sola finestra."""
        # Se già aperta, portala in primo piano invece di aprirne un'altra
        db = getattr(self, "_deep_browser_instance", None)
        if db and hasattr(db, "win") and db.win.winfo_exists():
            db.win.lift()
            db.win.focus_set()
        ...
        if not _DEEP_BROWSER_AVAILABLE:
            self._hud_alert("Modulo mancante",
                "File deep_browser.py non trovato.\n"
                f"Cercato in: {SCRIPT_DIR}\n"
    ...
    def _dnd_set_dest(self, key, preset_name, dropped):
        ...
        folders = [p for p in dropped if os.path.isdir(p)]
    ...
    def _warn_preset_duplicates(self, preset_name, new_path, new_key, parent=None):
        """Mostra toast di avviso se il percorso è già usato in un altro tasto."""
        dups = self._check_preset_duplicates(preset_name, new_path, new_key)
        if dups:
            dup_str = "+".join(dups)
            self._show_toast(
                tk_safe(f"Attenzione: percorso già usato nel tasto {dup_str}"),
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.isfile(arg):
            ...
            tk.messagebox.showerror("Errore", f"Percorso non valido:\n{arg}", parent=root)
    else:
